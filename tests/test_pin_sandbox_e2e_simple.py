"""
Simplified end-to-end test for pin-sandbox functionality using official MCP client.
...
This is a more focused test that can be run independently.
"""
...
class SimplePinSandboxE2ETest:
    ...
    async def setup_mcp_server(self):
        ...
        print(f"🔧 Setting up MCP server for test ID: {self.test_id}")
        ...
        self.session, self.server_process = await self._client_context.__aenter__()
    ...
    async def cleanup_mcp_server(self):
        """Clean up MCP server and client."""
...
'''
        ...
        result = await test.session.call_tool("execute_code", {
